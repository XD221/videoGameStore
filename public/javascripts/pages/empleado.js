...
function mostrarModal() {
  limpiarDatos();
  var modal = "#agregar";
  $(modal).find(".modal-title").html("Agregar Empleado");
  $(modal)
    .find(".modal-dialog")
    .css({ width: 700 + "px" });
  $(modal)
...
function eliminarExitoso(respuesta, e, elemento) {
  ...
  if (respuesta.Success) {
    ...
    toastr.success(
      "El empleado: " + elemento.nombre + " se ha eliminado satisfactoriamente"
    );
  } else {
...
function mostrarEliminar(e, elemento) {
  var modal = "#eliminar";
  $(modal).find(".modal-title").html("Eliminar Empleado");
  $(modal)
    .find(".text-mensaje-modal")
    .html(
      "¿Estas seguro que deseas eliminar al Empleado: " + elemento.nombre + "?"
    );
  $(modal)
    .find(".modal-body")
...
function eventoActualizar(input, elemento) {
  $(input)
    ...
    .click(function () {
      ...
      $(modal)
        .find(".modal-title")
        .html("Editar Empleado: " + elemento.nombre);
      $(modal)
        .find(".modal-dialog")
        .css({ width: 700 + "px" });
      $(modal)
