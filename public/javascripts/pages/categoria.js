...
function mostrarModal() {
  limpiarDatos();
  var modal = "#agregar";
  $(modal).find(".modal-title").html("Agregar Categoria");
  $(modal)
    .find(".modal-dialog")
    .css({ width: 700 + "px" });
  $(modal)
...
function eliminarExitoso(respuesta, e, elemento) {
  ...
  if (respuesta.Success) {
    ...
    toastr.success(
      "La categoria: " + elemento.nombre + " se ha eliminado satisfactoriamente"
    );
  } else {
...
function mostrarEliminar(e, elemento) {
  ...
  $(modal)
    ...
    .html(
      "¿Estas seguro que deseas eliminar la Categoria: " + elemento.nombre + "?"
    );
  $(modal)
    .find(".modal-body")
...
function eventoActualizar(input, elemento) {
  $(input)
    ...
    .click(function () {
      ...
      $(modal)
        .find(".modal-title")
        .html("Editar Categoria: " + elemento.nombre);
      $(modal)
        .find(".modal-dialog")
        .css({ width: 700 + "px" });
      $(modal)
...
function guardarExitoso(respuesta, elemento) {
  if (respuesta.Success) {
    ...
    if (!elemento) {
      ...
      elemento.descripcion = $("#descripcion").val();
    }
    mostrarDatos();
    toastr.success("La categoria se ha guardado correctamente");
  } else {
    toastr.error(respuesta.Message);
  }
}
